...
#define CheckPtr(obj, ret) if (obj == nullptr) { LOG_ERROR("[API] Given Pointer was NULL!"); return ret; }
	...
	*/
	...
	// Baked per-vertex terrain color (lighting/AO from the original renderer).
	// Returned as raw RGBA bytes, 4 per vertex.
	const void Terrain_GetColorBuffer(const Terrain *terr, uint32_t& count, uint8_t*& colors)
	{
		count = 0;
		CheckPtr(terr, );
		Color4u8* colorBuffer = nullptr;
		terr -> GetColorBuffer(count, colorBuffer);
		colors = reinterpret_cast<uint8_t*>(colorBuffer);
	}
