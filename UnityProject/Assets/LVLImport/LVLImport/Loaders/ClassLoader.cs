...
public class ClassLoader : Loader
{
    ...
    public GameObject Instantiate(ISWBFProperties instOrClass, string instName)
    {
        ...
        if (!IsGeometryManuallyInitialized(instOrClass) && instOrClass.GetProperty("GeometryName", out string geometryName))
        {
            ...
            if (obj == null)
            {
                // Error, not warning: the instance still gets created as an
                // empty GameObject - invisible but fully collidable, which for
                // a soldier reads in-game as "I spawned but I don't exist".
                Debug.LogErrorFormat("Failed to load model '{1}' used by object '{0}' - it will be INVISIBLE. " +
                                     "Usually the .lvl containing the model was not mounted (wrong addon data path?).",
                                     instName, geometryName);
                BFImportDiagnostics.Missing(BFSourceKind.Model, geometryName, instName);
