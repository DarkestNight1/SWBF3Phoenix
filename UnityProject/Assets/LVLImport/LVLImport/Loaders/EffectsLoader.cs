...
public class EffectsLoader : Loader {
    ...
    /// <summary>
    /// The most of the screen a single particle may cover.
    /// </summary>
    /// <remarks>
    /// <c>maxParticleSize</c> is a fraction of the viewport, not a world size,
    /// and it was set to 999999 - which removes the clamp that exists
    /// precisely to stop a particle near the camera from scaling up to fill
    /// the frame. That is what produces a screen of flat colour with a hard
    /// straight edge across it: the edge is the billboard quad, and because
    /// the quad sits between the camera and everything else, the effect also
    /// appears to be drawing through walls.
    ///
    /// Unity's own default is 0.5. This is a little more generous so a close
    /// explosion still reads as large, while a single particle can no longer
    /// become the whole image.
    /// </remarks>
    const float MaxParticleScreenFraction = 0.6f;
    ...
    public GameObject ImportEffect(string name)
    {
        ...
        if (fx == null)
        {
            ...
            BFImportDiagnostics.Missing(BFSourceKind.Effect, name);
            return null;
        }
        ...
        if (psObj != null)
        {
            BFImportDiagnostics.Resolved(BFSourceKind.Effect, name);
        }
        else
        {
            // The .fx config exists but conversion produced nothing, which is a
            // different failure from a missing effect and just as invisible.
            BFImportDiagnostics.Missing(BFSourceKind.Effect, name, "conversion produced no object");
        }
        return psObj;
    }
    ...
    private GameObject GetEmitter(Field emitter, int ParentMaxParticles = 1)
    {
        ...
        */
        ...
        if (!HasEmptySpawner || HasVelTransformation)
        {
            ...
            ApplyVelocityModule(velModule, VelCurves, scaleCurveOut);
        ...
        */
        ...
        else if (geomType.Equals("BILLBOARD", StringComparison.OrdinalIgnoreCase))
        {
            ...
            psR.maxParticleSize = MaxParticleScreenFraction;
        }  
    ...
    /// </summary>
    /// <summary>
    /// Fill in a velocity-over-lifetime module with every curve in one mode.
    /// </summary>
    /// <remarks>
    /// Unity requires that <b>every</b> curve in this module share a
    /// MinMaxCurve mode - not just x, y and z. The module also owns
    /// speedModifier, orbitalX/Y/Z, orbitalOffsetX/Y/Z and radial, and those
    /// default to Constant. So matching only the three axes and then assigning
    /// a speedModifier the converter produced as a curve still trips
    /// "Particle Velocity curves must all be in the same mode" - which is why
    /// that warning kept filling the console after the axes were already being
    /// matched, and why the effect still silently lost its velocity.
    ///
    /// Every curve the module has is therefore brought to one mode here, with
    /// the ones the stock data never specifies written as mode-appropriate
    /// zeroes rather than left at their defaults.
    /// </remarks>
    static void ApplyVelocityModule(ParticleSystem.VelocityOverLifetimeModule module,
                                    ParticleSystem.MinMaxCurve[] axes,
                                    ParticleSystem.MinMaxCurve speedModifier)
    {
        if (axes == null || axes.Length < 3) return;

        var all = new ParticleSystem.MinMaxCurve[4];
        all[0] = axes[0];
        all[1] = axes[1];
        all[2] = axes[2];
        all[3] = speedModifier;
        ...
        module.y = all[1];
        module.z = all[2];
        module.speedModifier = all[3];

        // Orbital and radial motion are not expressible in the source format,
        // so they are zero - but they must be zero *in the module's mode*, not
        // in whatever mode a default-constructed MinMaxCurve happens to use.
        ParticleSystem.MinMaxCurve zero = ZeroInMode(all[0].mode);
        module.orbitalX = zero;
        module.orbitalY = zero;
        module.orbitalZ = zero;
        module.orbitalOffsetX = zero;
        module.orbitalOffsetY = zero;
        module.orbitalOffsetZ = zero;
        module.radial = zero;
    }

    static ParticleSystem.MinMaxCurve ZeroInMode(ParticleSystemCurveMode mode)
    {
        switch (mode)
        {
            ...
            case ParticleSystemCurveMode.Curve:
                return new ParticleSystem.MinMaxCurve(0f, AnimationCurve.Constant(0f, 1f, 0f));
            case ParticleSystemCurveMode.TwoCurves:
                return new ParticleSystem.MinMaxCurve(0f,
                    AnimationCurve.Constant(0f, 1f, 0f), AnimationCurve.Constant(0f, 1f, 0f));
            default:
                return new ParticleSystem.MinMaxCurve(0f);
        }
    }
