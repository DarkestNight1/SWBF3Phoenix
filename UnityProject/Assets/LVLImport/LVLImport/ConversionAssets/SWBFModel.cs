...
public class SWBFCollider
{
    ...
    /// <summary>
    /// The "collides with everything" layer for a game role. Used as the
    /// fallback when a collider's mask flags don't map to a specific layer,
    /// so unrecognised collision still blocks movement instead of vanishing.
    /// </summary>
    public static int RoleDefaultLayer(SWBFGameRole GameRole)
    {
        switch (GameRole)
        {
            case SWBFGameRole.Vehicle:  return LayerMask.NameToLayer("VehicleAll");
            case SWBFGameRole.Soldier:  return LayerMask.NameToLayer("SoldierAll");
            case SWBFGameRole.Terrain:  return LayerMask.NameToLayer("TerrainAll");
            case SWBFGameRole.Ordnance: return LayerMask.NameToLayer("OrdnanceAll");
            default:                    return LayerMask.NameToLayer("BuildingAll");
        }
    }
...
public class SWBFModel
{
    ...
    // One warning per distinct collision node name; a map has thousands of
    // colliders and an unmapped mask is usually systemic, not a one-off.
    static readonly HashSet<string> ReportedUnmappedMask = new HashSet<string>();
    ...
    public void SetColliderLayerFromMaskAll(bool Print = false)
    {
        foreach (SWBFCollider Collider in Colliders)
        {
            ...
            if (Layer == -1)
            {
                // A mask we can't map is NOT a reason to delete collision from
                // the world. MapRoleAndMaskToLayer only recognises the All /
                // Ordnance / Soldier / Vehicle flags, so a collider flagged
                // only Building(4), Terrain(8) or Flag(32) - or one whose mask
                // parsed as 0 - fell through every branch to -1. Deactivating
                // the node then left interior floors and walls with no
                // collision at all, and soldiers dropped straight through the
                // building to the terrain below.
                //
                // Falling back to the role's "All" layer keeps the geometry
                // solid, which is the right default for level collision: an
                // over-solid collider is a far cheaper mistake than a floor
                // that isn't there.
                Layer = SWBFCollider.RoleDefaultLayer(GameRole);
                if (ReportedUnmappedMask.Add(Collider.Node.name))
                {
                    Debug.LogWarning($"Collision node '{Collider.Node.name}' (role {GameRole}, " +
                                     $"mask {(uint)Collider.Mask}) has no layer mapping - keeping it " +
                                     $"solid on {LayerMask.LayerToName(Layer)} rather than disabling it.");
                }
            }
